...
light.position.set( 2*N, 4*N, 2*N );
...
var brickStyles = [
	...
	{ // cartoon bricks
		scale: -0.75,
		brickSize: new THREE.Vector3( 2, 3, 6 ),
		brickShift: 2,
		jointSize: 0.3,
		jointSpan: 0.9,
		jointJitter: 0.9,
...
var initGeom = new THREE.BoxGeometry( 2*N, N, 2*N, 2*N, N, 2*N );
...
function nextShape() {
	...
	new TWEEN.Tween( data )
		.to( { time: data.time+0.6, strength: 0.5+Math.random()**2 }, 600 )
...
function animationLoop( t ) {
	...
	cubix.rotation.z = ( t + Math.sin( t ) )/2+Math.PI/2;
